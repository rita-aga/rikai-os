...
mod dst_tests {
    ...
    async fn test_fault_injection_on_get() {
        let backend = SimStorageBackend::new(SimConfig::with_seed(42)).with_faults(
            FaultConfig::new(FaultType::StorageReadFail, 1.0).with_filter("get"),
        );
    ...
    async fn test_fault_injection_probability() {
        ...
        let backend = SimStorageBackend::new(SimConfig::with_seed(42)).with_faults(
            FaultConfig::new(FaultType::StorageWriteFail, 0.5).with_filter("store"),
        );
...
mod property_tests {
    ...
    #[derive(Debug, Clone)]
    enum StorageOp {
        Store { entity_type: EntityType, name: String },
        Get { id: String },
        Delete { id: String },
        Search { query: String },
        List { entity_type: Option<EntityType> },
    }
    ...
    impl PropertyTestable for StorageWrapper {
            ...
            rt.block_on(async {
                match op {
                    StorageOp::Store { entity_type, name } => {
                        let entity =
                            Entity::new(*entity_type, name.clone(), "content".to_string());
        ...
        fn check_invariants(&self) -> Result<(), String> {
            ...
            rt.block_on(async {
                ...
                let count = self.backend.count_entities(None).await.map_err(|e| e.to_string())?;
