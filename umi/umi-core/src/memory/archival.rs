...
mod tests {
    ...
    async fn test_remember_auto_name() {
        ...
        let entity = memory
            .remember("This is a note about something important", EntityType::Note, None)
            .await
            .unwrap();
    ...
    async fn test_recall_finds_matching() {
        ...
        memory
            .remember("Alice is a software engineer", EntityType::Person, Some("Alice"))
            .await
            .unwrap();
        memory
    ...
    async fn test_list_by_type() {
        ...
        let projects = memory.list(Some(EntityType::Project), 100, 0).await.unwrap();
...
mod dst_tests {
    ...
    async fn test_remember_with_fault_injection() {
        let backend = SimStorageBackend::new(SimConfig::with_seed(42)).with_faults(
            FaultConfig::new(FaultType::StorageWriteFail, 1.0).with_filter("store"),
        );
...
mod property_tests {
    use super::*;
    use crate::dst::{DeterministicRng, PropertyTest, PropertyTestable, SimClock, SimConfig, TimeAdvanceConfig};
    use crate::storage::SimStorageBackend;

    /// Operations on ArchivalMemory
    #[derive(Debug, Clone)]
    enum MemoryOp {
        Remember { content: String, entity_type: EntityType },
        Recall { query: String },
        Forget { id: String },
        Get { id: String },
        Count,
    ...
    impl PropertyTestable for MemoryWrapper {
        ...
        fn apply_operation(&mut self, op: &Self::Operation, _clock: &SimClock) {
            ...
            rt.block_on(async {
                match op {
                    MemoryOp::Remember { content, entity_type } => {
